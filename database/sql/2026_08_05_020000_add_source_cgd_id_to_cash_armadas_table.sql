...
--
-- Audit trail untuk CashArmada row yang auto-dibuat oleh
-- CashGudang::acceptCashGudang() -- menelusuri balik ke cash_gudang_details
-- asalnya. Nullable: hanya diisi untuk row yang lahir dari alur ini.
...
SET @sql := IF(
  ...
  'ALTER TABLE `cash_armadas`
     ADD COLUMN `source_cgd_id` INT UNSIGNED NULL DEFAULT NULL AFTER `cash_id`',
