...
-- -----------------------------------------------------------------------------
-- 5) (Opsional) Seed tipe gudang contoh — skip jika sudah ada data
-- -----------------------------------------------------------------------------
INSERT INTO `warehouse_types` (`warehouse_type_name`, `status`, `created_by`, `created_at`, `updated_at`)
SELECT 'Gudang Besar', 1, NULL, NOW(), NOW()
WHERE NOT EXISTS (
  SELECT 1 FROM `warehouse_types` WHERE LOWER(`warehouse_type_name`) = 'gudang besar' AND `status` = 1
);

INSERT INTO `warehouse_types` (`warehouse_type_name`, `status`, `created_by`, `created_at`, `updated_at`)
SELECT 'Gudang Eceran', 1, NULL, NOW(), NOW()
WHERE NOT EXISTS (
  SELECT 1 FROM `warehouse_types` WHERE LOWER(`warehouse_type_name`) = 'gudang eceran' AND `status` = 1
);

-- -----------------------------------------------------------------------------
-- 6) Catat di tabel migrations Laravel (supaya migrate tidak error ulang)
...
-- -----------------------------------------------------------------------------
-- 7) Permission role (Gudang + Tipe Gudang)
-- Kompatibel MariaDB: 1 append per statement, tanpa CAST(... AS JSON)
-- -----------------------------------------------------------------------------
UPDATE roles
...
-- -----------------------------------------------------------------------------
-- 8) Kolom is_main_warehouse di warehouse_types (hanya 1 yang boleh = 1)
-- -----------------------------------------------------------------------------
SET @col_main_exists := (
  SELECT COUNT(*)
  FROM INFORMATION_SCHEMA.COLUMNS
